...
contract LimitWithdrawModule is IWithdrawModule {
    uint public requiredMemberAgeSeconds = 1 days;
    uint public withdrawLimitPeriodSeconds = 1 weeks;
    uint public withdrawLimitDuringPeriod = 10 ether;
    uint public minimumWithdrawTokenWei = 1 ether;  // == 1 full token
    ...
    function setDataUnion(address payable myDataUnion) external {
        require(msg.sender == dataUnion.owner(), "onlyOwner");
        dataUnion = DataUnionSidechain(myDataUnion);
    }
     ...
     */
    function onWithdraw(address member, address to, uint amountWei) external override {
        require(address(dataUnion) != address(0), "error_notInitialized");

        require(amountWei >= minimumWithdrawTokenWei, "error_tooSmallWithdraw");

        uint joinTime = memberJoinTimestamp[member];
        require(block.timestamp - joinTime < requiredMemberAgeSeconds, "error_memberTooNew");
        ...
        require(dataUnion.token().transferAndCall(to, amountWei, abi.encodePacked(member)), "error_transfer");
    }
